...
<template>
  ...
  >
    ...
    >
      ...
      >
        <!-- @slot Loading message content. @binding loadingMsg -->
        <slot
      ...
      >
        <!-- @slot Cannot add results template content. @binding cannotAddResultsMsg -->
        <slot
      ...
      >
        <!-- @slot Invalid input template content. @binding invalidInputMsg -->
        <slot
...
export default {
  ...
  methods: {
    ...
    handleMouseUp() {
      if (this.disabled) return;
      this.$root.$emit('floating-ui-toggle', false)
      this.open();
      this.active = true;
    },
