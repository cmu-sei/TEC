    ...
    <div class="p-6 bg-white">
      ...
      >
        <template #trigger="{ open, close }">
          <sds-button
            variant="primary"
            @mouseover.native="open(500)"
            @mouseout.native="close(500)"
          >
            Hello
          </sds-button>
        </template>
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'lastModified', label: 'Last Modified', sortable: true, format: (date) => date.toLocaleDateString() },
  ...
  watch: {
    fileUploaderModel(value) {
      this.uploadedImages = value.map((file) => {
        return {
